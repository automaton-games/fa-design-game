...
**alphabet は問題データが所有する。** 提出DFAに`alphabet`は含めない。問題詳細APIが返す入力アルファベットを、サーバーが提出DFAへ適用する。提出JSONに`alphabet`が含まれていた場合は未知フィールドとして拒否する。
...
入力アルファベットは問題データから渡され、`dfa.Validate`がDFA本体と同時に検証する。1要素以上、重複なしとし、空文字・空白のみの入力記号を拒否する。複数文字の入力記号は許可し、長さや使用文字には追加制限を設けない。反例は記号を連結した文字列ではなく、入力記号の配列として返す。
